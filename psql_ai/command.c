             ...
             PQExpBuffer previous_buf)
{
    ...
    {
        char *prompt = psql_scan_slash_option(scan_state, OT_NORMAL, NULL, false);
        PGresult *res;

        if (!active_branch)
        {
            /* Skip execution in inactive branch */
            if (prompt)
                pfree(prompt);
        ...
        {
            pg_log_error("No database connection");
            if (prompt)
                pfree(prompt);
            status = PSQL_CMD_ERROR;
        }
        else if (!prompt || strlen(prompt) == 0)
        {
            printf("Welcome to AI Chat! Enter a prompt after '/ai', e.g., '/ai Tell me about the Taj Mahal'\n");
            if (prompt)
                pfree(prompt);
            status = PSQL_CMD_SKIP_LINE;
        }
        else
        {
            ...
            char query[1024];
            snprintf(query, sizeof(query), "SELECT ai_chat_response('%s')", prompt);
            res = PQexec(pset.db, query);

            if (PQresultStatus(res) != PGRES_TUPLES_OK)
            {
                pg_log_error("Query failed: %s", PQerrorMessage(pset.db));
                PQclear(res);
                status = PSQL_CMD_ERROR;
            }
            else if (PQntuples(res) > 0 && PQnfields(res) > 0 && PQgetvalue(res, 0, 0))
            {
                printf("AI: %s\n", PQgetvalue(res, 0, 0));
                status = PSQL_CMD_SKIP_LINE; /* No further query execution needed */
            }
            else
            {
                printf("AI: No response generated\n");
                status = PSQL_CMD_SKIP_LINE;
            }
            PQclear(res);
            if (prompt)
